...
service.interceptors.request.use(
  config => {
    ...
    if (store.getters.token) {
      // 让每个请求携带token-- ['X-Store-Admin-Token']为自定义key 请根据实际情况自行修改
      config.headers['X-Store-Admin-Token'] = getToken()
    }
    return config
  },
  error => {
...
service.interceptors.response.use(
  response => {
    ...
    if (res.errno === 501) {
      ...
      return Promise.reject('error')
    } else if (res.errno !== 200 && response.request.responseType != "blob") {
